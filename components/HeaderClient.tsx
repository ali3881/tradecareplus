...
import { Phone, Search, X, Wrench, Flame, Clock, Mail, ChevronDown, MapPin } from "lucide-react";
...
function DesktopNav({
  ...
  const serviceIsActive = pathname === "/services" || pathname.startsWith("/service/");

  const renderNavLink = (item: { href: string; label: string }, index: number, total: number) => {
  ...
  return (
    <div className="flex justify-between items-center bg-yellow-400">
      <nav className="flex items-center h-full font-semibold text-gray-800">
        {renderNavLink(NAV_ITEMS[0], 0, 2)}

        <div className="group relative h-full">
          <Link
            href="/services"
            className={`py-6 px-6 leading-[1.8] capitalize tracking-[1px] transition-colors h-full flex items-center gap-2 pr-7 relative ${
              serviceIsActive ? "text-red-600" : "hover:text-red-600"
            } before:content-[''] before:absolute before:right-0 before:top-0 before:bottom-[1px] before:w-[8px] before:h-[1px] before:bg-white before:my-auto before:z-[1] before:rotate-[-35deg] after:content-[''] after:absolute after:right-0 after:top-0 after:bottom-0 after:h-[8px] after:w-[8px] after:rounded-full after:bg-black after:my-auto after:opacity-80 after:transition-all after:duration-300 after:ease-linear`}
          >
            <span>Services</span>
          ...
          <div className="invisible absolute left-0 top-full z-[260] min-w-[320px] translate-y-2 border border-yellow-200 bg-white opacity-0 shadow-[0_18px_40px_rgba(0,0,0,0.16)] transition-all duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100">
            <div className="max-h-[420px] overflow-y-auto py-2">
              ...
              {services.map((service) => (
                <Link
                  ...
                  className="block px-5 py-3 text-sm text-stone-700 transition-colors hover:bg-yellow-50 hover:text-red-600"
                >
                  {service.title}
                </Link>
              ))}
        ...
        {NAV_ITEMS.slice(1).map((item, index) => renderNavLink(item, index, NAV_ITEMS.length - 1))}
      </nav>
...
export default function HeaderClient({ services }: { services: ServiceNavItem[] }) {
  ...
  const servicesActive = pathname === "/services" || pathname.startsWith("/service/");
  ...
  return (
    <header className="w-full relative z-[120]">
      <div className="hidden md:block py-3 bg-[#111111] text-white text-sm uppercase">
        <div className="max-w-[1280px] mx-auto px-6 flex items-end justify-end">
          <div className="flex items-center gap-[40px]">
            <Link href="/technician-map" className="flex items-center gap-3 text-yellow-400 hover:text-white transition-colors">
              <MapPin className="text-white" size={14} />
              <span>Technician Locator</span>
            </Link>
      ...
      <div className="max-w-[1290px] mx-auto lg:px-6 hidden md:block bg-transparent -mb-7 relative z-20">
        <div className="bg-yellow-400 px-4 lg:px-0">
          ...
          <div className="hidden lg:block">
            <DesktopNav showUserMenu pathname={pathname} services={services} />
          </div>
        </div>
      </div>
      ...
      >
        ...
        <aside
          className={`absolute right-0 top-0 h-[100dvh] w-[280px] bg-black text-white shadow-2xl transition-transform duration-300 ease-out ${
          ...
          <nav className="pt-12 flex flex-col">
            <Link
              ...
              className={`px-4 py-3 border-b border-white/15 transition-colors ${
                isActivePath(NAV_ITEMS[0].href) ? "bg-yellow-400 text-red-600" : "hover:bg-yellow-400 hover:text-red-600"
              }`}
            >
              {NAV_ITEMS[0].label}
            </Link>
            ...
            <button
              ...
              className={`flex items-center justify-between px-4 py-3 border-b border-white/15 transition-colors ${
                servicesActive ? "bg-yellow-400 text-red-600" : "hover:bg-yellow-400 hover:text-red-600"
              }`}
            >
              <span>Services</span>
              ...
              <div className="bg-white/5">
                {services.map((service) => (
                  <Link
                    ...
                    href={`/service/${service.slug}`}
                    onClick={() => setIsMenuOpen(false)}
                    className={`block px-6 py-3 border-b border-white/10 text-sm transition-colors ${
                      pathname === `/service/${service.slug}` ? "bg-yellow-400 text-red-600" : "text-white/80 hover:bg-yellow-400 hover:text-red-600"
                    }`}
                  >
                    {service.title}
                  </Link>
            ...
            <Link
              ...
              onClick={() => setIsMenuOpen(false)}
              className={`px-4 py-3 border-b border-white/15 transition-colors ${
                isActivePath("/technician-map") ? "bg-yellow-400 text-red-600" : "hover:bg-yellow-400 hover:text-red-600"
              }`}
            >
              Technician Locator
            </Link>
