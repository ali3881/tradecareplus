...
def main():
    try:
        ...
        ls_out = run_command(ssh, f"ls -d {app_dir}", exit_on_fail=False, verbose=False)
        if "No such file" in ls_out:
            log("Creating directory...")
            run_command(ssh, f"mkdir -p {app_dir}")
        
        # Check if package.json exists
        pkg_check = run_command(ssh, f"test -f {app_dir}/package.json", exit_on_fail=False)
        if pkg_check: # Exit status 0 means success (file exists)
             # Proceed
             pass
        else:
             log("Project not uploaded! Cannot proceed.")
             sys.exit(1)
